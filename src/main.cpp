...
std::vector<uint8_t> patternOuterIn(int n) {
    ...
    while (left <= right) {
        v.push_back(left);
        if (left != right) v.push_back(right);
        ++left; --right;
    }
    return v;
}
...
std::vector<uint8_t> patternInwardBounce(int n) {
    ...
    for (int offset = 1; mid - offset >= 0 || mid + offset < n; ++offset) {
        if (mid - offset >= 0) v.push_back(mid - offset);
        if (mid + offset < n) v.push_back(mid + offset);
    }
    return v;
}
...
std::vector<uint8_t> patternSpiral(int n) {
    ...
    while (left <= right) {
        if (i % 2 == 0) v.push_back(left++);
        else v.push_back(right--);
        ++i;
    }
    return v;
}
...
std::vector<uint8_t> patternJumpStep(int n) {
    ...
    for (int i = 0; i < half; ++i) {
        v.push_back(i);
        if (i + half < n) v.push_back(i + half);
    }
    return v;
}
...
std::vector<uint8_t> patternSkipReverse(int n) {
    ...
    for (int i = n - 2; i >= 0; i -= 2) v.push_back(i);
    return v;
}
...
std::vector<uint8_t> patternBackwardJump(int n) {
    std::vector<uint8_t> v;
    for (int i = n - 1; i >= 0; i -= 3) v.push_back(i);
    for (int i = n - 2; i >= 0; i -= 3) v.push_back(i);
    return v;
}
