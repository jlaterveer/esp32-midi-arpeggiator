...
void rotary_init()
{
  pinModeFast(encoder0PinA, INPUT_PULLUP);
  pinModeFast(encoder0PinB, INPUT_PULLUP);
}
unsigned char rotary_process()
{
  unsigned char pinstate = (digitalReadFast(encoder0PinA) << 1) | digitalReadFast(encoder0PinB);
